...
copy_globs "${REMOTE_LIB}" "libasound*.so* libpulse*.so* libsamplerate*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libasound*.so* libpulse*.so* libsamplerate*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
copy_globs "${REMOTE_LIB}" "libdrm*.so* libX11*.so* libxcb*.so* libXau*.so* libXdmcp*.so* libXext*.so* libXfixes*.so* libXrender*.so* libXi*.so* libXrandr*.so* libXcursor*.so* libXss*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libdrm*.so* libX11*.so* libxcb*.so* libXau*.so* libXdmcp*.so* libXext*.so* libXfixes*.so* libXrender*.so* libXi*.so* libXrandr*.so* libXcursor*.so* libXss*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
copy_globs "${REMOTE_LIB}" "libgbm*.so* libdecor*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libgbm*.so* libdecor*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
copy_globs "${REMOTE_LIB}" "libwayland-client*.so* libwayland-egl*.so* libwayland-cursor*.so* libxkbcommon*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libwayland-client*.so* libwayland-egl*.so* libwayland-cursor*.so* libxkbcommon*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
...
copy_globs "${REMOTE_LIB}" "libbsd*.so* libmd*.so* libglib-2.0*.so* libgobject*.so* libgio*.so* libgmodule*.so* libxml2*.so* libcairo*.so* libsndfile*.so* libasyncns*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libbsd*.so* libmd*.so* libglib-2.0*.so* libgobject*.so* libgio*.so* libgmodule*.so* libxml2*.so* libcairo*.so* libsndfile*.so* libasyncns*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
copy_globs "${REMOTE_LIB}" "libgcrypt*.so* libgmp*.so* libnettle*.so* libhogweed*.so* libtasn1*.so* libidn2*.so* libp11-kit*.so* libunistring*.so* libffi*.so* libgnutls*.so* libFLAC*.so* libcap*.so* liblz4*.so* libzstd*.so* libogg*.so* libvorbis*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libgcrypt*.so* libgmp*.so* libnettle*.so* libhogweed*.so* libtasn1*.so* libidn2*.so* libp11-kit*.so* libunistring*.so* libffi*.so* libgnutls*.so* libFLAC*.so* libcap*.so* liblz4*.so* libzstd*.so* libogg*.so* libvorbis*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
copy_globs "${REMOTE_LIB}" "libsystemd*.so* libapparmor*.so*" "${SYSROOT}${REMOTE_LIB}"
copy_globs "${REMOTE_LIB_ALT}" "libsystemd*.so* libapparmor*.so*" "${SYSROOT}${REMOTE_LIB_ALT}"
